...
for i=[1:9,11:19,21:length(lfp{a})]
                ...
                if ~(prot=='C') tf=1; end
                    if tf==1
                       [sr sl]=ReadStartTime(pos);
                        ...
                        figure(i)
                        subplot(2,2,1)
                        [meanp peakp]=COMODOphase2(eeg',n_fs,9,3,18,'All');
                        COMODOphase2(eeg',n_fs,10,3,18,'All');
                        subplot(2,2,2)
                        [meanp1 peakp1]=COMODOphase2(x1,n_fs,9,3,18,'Resting');
                        COMODOphase2(x1,n_fs,10,3,18,'Resting');
                        subplot(2,2,3)
                        [meanp2 peakp2]=COMODOphase2(x2,n_fs,9,3,18,'Waiting');
                        COMODOphase2(x2,n_fs,10,3,18,'Waiting');
                        subplot(2,2,4)
                        [meanp3 peakp3]=COMODOphase2(x3,n_fs,9,3,18,'Running');
                        COMODOphase2(x3,n_fs,10,3,18,'Running');
                        saveas(i,strcat('/home/melisa/Escritorio/Melisa/Doctorado/BandsCoupling/Modulation2/States/',name(a,:),'_',lfp{a}(i,:)),'png')
                        close(i)
...
nombre=strcat('/home/melisa/Escritorio/Melisa/Doctorado/BandsCoupling/Modulation2/States/',int2str(prot),'_',name(a,:),'_COMODO_mean_states');
